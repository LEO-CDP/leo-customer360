...
- **PostgreSQL:** identities, raw-profile staging, CRM data, operational aggregates, object manifests, and processing state.
...
|---|---|---|---|
| Public tracking ingestion | `data-tracking-api` validates batches, publishes to Redis Streams, then writes NDJSON to S3 | S3 object | Extend envelope, manifest, compaction, and durable processing state |
...
| Dagster analytics | Scans S3 JSONL and updates Redis plus `sys_data_source` totals | S3 + Redis + PostgreSQL aggregates | Add manifest, compaction, validation, replay, and reconciliation |
...
flowchart LR
    ...
    DAGSTER --> MANIFEST[(PostgreSQL object manifest)]
...
| Event object status/checksum | PostgreSQL manifest | Mutable control-plane state |
...
### 4.3 Durable manifest

Add a small PostgreSQL control table. It must never contain one row per event.

```text
event_object_manifest
...
Recommended statuses: `received`, `validated`, `compacted`, `quarantined`, `failed`, `replayed`.

The manifest is the durable processing ledger. Redis processed-object keys and cursors remain useful caches, but cannot be the recovery authority because they expire.
...
- [ ] Confirm current writers, readers, S3 buckets, Redis stream names, Dagster jobs, and deployment environment variables.
- [ ] Record a baseline count and time range for PostgreSQL `cdp_raw_events`.
- [ ] Record S3 object counts, byte totals, event counts, and failed/pending Redis messages.
- [ ] Decide Bronze format (`jsonl.gz` recommended for the first cutover) and Silver format (`Parquet` first; `Iceberg` only if snapshot/schema-evolution requirements justify it).
- [ ] Approve the event envelope and deduplication contract.
- [ ] Approve the S3 bucket/key layout and retention policy for UAT and PROD.
- [ ] Define tenant isolation tests for both object prefixes and query results.

**Gate:** contract, retention, query engine, and rollback owner are documented before code changes begin.

### Phase 1: Durable S3 ingestion

- [ ] Add event ID generation, schema versioning, normalized timestamps, and deterministic deduplication to the S3 ingestion path.
- [ ] Add a manifest write/update path with idempotent `(bucket, object_key)` handling.
- [ ] Keep Redis Streams acknowledged only after the S3 write succeeds.
- [ ] Bound request, batch, object, and queue sizes; expose queue depth and oldest-pending age metrics.
- [ ] Add upload checksum and event-count metadata.
- [ ] Add quarantine handling for malformed or unauthorized events.
- [ ] Make retries safe: the same Redis message or HTTP retry must not create a second logical event.

**Gate:** kill/restart workers during upload and prove that no acknowledged event is lost and no retry creates an unintended duplicate.

### Phase 2: Route the legacy event API to S3

- [ ] Preserve `cdp_raw_profiles_stage` resolution and validation in `customer360-api`.
- [ ] Replace direct `CdpRawEvent` insertion in `/events` and `/events/bulk` with the shared S3 ingestion contract.
- [ ] Return `202 Accepted` with event/batch/object identifiers after durable enqueue, not after a PostgreSQL event insert.
- [ ] Keep a compatibility response shape until clients migrate.
- [ ] Add feature flags for `EVENT_WRITE_BACKEND=postgres|dual|s3`.
- [ ] During dual-write, compare event IDs, counts, timestamps, tenant IDs, dedup keys, and payload hashes.

**Gate:** all supported writers can send events to S3, and the dual-write discrepancy rate is zero or explicitly explained.

### Phase 3: Silver compaction and query service

- [ ] Add a Dagster compaction job that discovers manifest rows, validates Bronze objects, deduplicates, and writes partitioned Silver data.
- [ ] Update the manifest only after the Silver write and checksum verification succeed.
- [ ] Quarantine malformed objects without blocking unrelated tenants or hours.
- [ ] Choose and implement the query engine: DuckDB/Polars for low-concurrency service reads, or Trino/ClickHouse/dedicated query service for higher concurrency.
- [ ] Enforce tenant and time-range predicates before querying S3.
- [ ] Use cursor pagination; never use unbounded `OFFSET` on event history.
- [ ] Return only the fields needed by the Customer 360 UI.
- [ ] Add query latency, scanned-byte, object-count, and error metrics.

**Gate:** the profile timeline and aggregate endpoints return equivalent results from S3 Silver and PostgreSQL for the comparison window.

### Phase 4: Identity enrichment and backfill

- [ ] Keep CIR based on `cdp_raw_profiles_stage`, `cdp_profile_links`, and master profiles unless a measured requirement proves otherwise.
- [ ] Do not mutate Bronze when a `master_profile_id` becomes known.
- [ ] Choose one enrichment strategy: versioned Silver rewrite, event identity mapping in S3, or a small recent-event projection in PostgreSQL.
- [ ] Export existing PostgreSQL events month-by-month and stream them to S3.
- [ ] Register every backfill object in the manifest.
- [ ] Compare source/destination counts, checksums, date ranges, tenants, and deduplication results.
- [ ] Make backfill resumable by partition and object, with a dry-run mode.

**Gate:** every PostgreSQL event partition selected for migration has a reconciled S3 representation and a tested replay path.

### Phase 5: Read and write cutover

- [ ] Enable S3 shadow reads and compare response payloads with PostgreSQL.
- [ ] Switch analytics reads to S3 Silver.
- [ ] Switch profile timeline and event reads to the query service.
- [ ] Switch all event writers to S3-only.
- [ ] Keep PostgreSQL `cdp_raw_events` read-only during the rollback window.
- [ ] Monitor event acceptance, manifest lag, query latency, discrepancy counts, and failed/quarantined objects.
- [ ] Document the rollback switch and rehearse it.

**Gate:** one complete retention window passes with no unexplained data loss, unacceptable query regression, or unresolved tenant-isolation finding.

### Phase 6: Remove PostgreSQL event storage

- [ ] Confirm no production writer or reader references `cdp_raw_events`.
- [ ] Archive the PostgreSQL table or final partitions according to the retention policy.
- [ ] Add a forward migration that drops the old table only after the archive and S3 reconciliation are approved.
- [ ] Remove the ORM model, direct event CRUD, raw-event SQL, partition function, indexes, and RLS list entry.
- [ ] Remove or rewrite PostgreSQL event tests and E2E assertions.
- [ ] Update operational checks, schema documentation, and data-source diagrams.

**Gate:** production rollback no longer depends on the dropped table; the S3 replay procedure is the recovery path.
...
| [backend-system/analytics/source_analytics/tracking_log_aggregation.py](../../backend-system/analytics/source_analytics/tracking_log_aggregation.py) | Process manifests, compact Silver data, reconcile counts, and retain Redis as cache only | 1, 3 |
...
| [database-init/database-schema.sql](../../database-init/database-schema.sql) | Add manifest/control tables during migration; remove raw-event table only in Phase 6 | 1, 6 |
| [database-init/migrations/001_harden_tenant_rls_policies.sql](../../database-init/migrations/001_harden_tenant_rls_policies.sql) | Include new manifest/control tables if tenant-scoped | 1 |
...
| [data-tracking-api/core/config.py](../../data-tracking-api/core/config.py) | Add envelope, bucket, compression, and manifest settings | 1 |
| [backend-system/analytics/source_analytics/](../../backend-system/analytics/source_analytics/) | Add compaction, manifest, reconciliation, and backfill modules | 3, 4 |
| [deployments/server/deploy-tracking.sh](../../deployments/server/deploy-tracking.sh) | Inject production S3 bucket, prefix, manifest, and observability settings | 1, 5 |
...
| `backend-system/analytics/source_analytics/event_manifest.py` | Manifest writes, status transitions, checksum validation, and idempotency |
...
| `backend-system/analytics/source_analytics/event_backfill.py` | Partitioned PostgreSQL export to S3 with resumable checkpoints |
| `database-init/migrations/<next>_event_object_manifest.sql` | Production migration for the control-plane manifest table |
...
| `backend-system/analytics/tests/test_event_compaction.py` | Validation, compaction, quarantine, manifest, and reconciliation tests |
...
| [docs/operations/database/check-db-data.sql](../operations/database/check-db-data.sql) | Replace raw-event row checks with manifest, aggregate, lag, and reconciliation checks |
| [all-data-simulator/README.md](../../all-data-simulator/README.md) | Verify S3 contents, compaction output, and query results rather than requiring a matching PostgreSQL event row |
| [all-data-simulator/run_tracking_analytics_e2e.sh](../../all-data-simulator/run_tracking_analytics_e2e.sh) | Change E2E validation to assert Bronze upload, Silver materialization, manifest state, and aggregate correctness |
...
- [ ] Manifest transitions are idempotent and do not regress completed objects.
- [ ] Invalid records are quarantined with tenant-safe paths.
- [ ] Compaction produces the expected Parquet/Iceberg schema and partition layout.
- [ ] Backfill resumes after interruption without skipping or duplicating a partition.
- [ ] Query filters always include tenant scope and a bounded time predicate.
...
- [ ] Run Dagster compaction; verify Silver object, manifest status, and counts.
...
- [ ] Manifest lag, queue depth, oldest pending message, quarantine count, compaction lag, and query latency are monitored.
...
EVENT_S3_PREFIX=bronze/events
EVENT_SILVER_PREFIX=silver/events/v1
EVENT_MANIFEST_ENABLED=true
```

Rollout order:

1. Deploy the envelope and manifest without changing existing reads.
2. Enable S3 shadow writes and reconcile with PostgreSQL.
3. Enable Silver compaction and query shadow reads.
4. Switch analytics reads to S3.
5. Switch profile timeline reads to S3.
6. Switch all event writers to S3-only.
7. Keep PostgreSQL event partitions read-only for one complete retention window.
8. Archive and drop PostgreSQL event storage in a separate release.

Rollback before the drop:

1. Set `EVENT_WRITE_BACKEND=dual` or `postgres`.
2. Set `EVENT_READ_BACKEND=postgres`.
3. Stop or pause compaction only if it is corrupting Silver output; retain Bronze objects.
4. Reconcile the cutover interval from S3 before resuming normal operation.
...
- [ ] Bronze objects, Silver partitions, manifests, and replay jobs are operational.
...
- [ ] Remove `CdpRawEvent` and its schemas/routes or convert the route to the S3 query contract.
