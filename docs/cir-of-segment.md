...
"@type": TechArticle
"@id": https://www.twilio.com/docs/segment/unify/identity-resolution/identity-resolution-settings#article
headline: Identity Resolution Settings
description: Configure Segment Identity Graph rules, external IDs, matching logic, identifier limits, priorities, and blocked values to control profile merges.
url: https://www.twilio.com/docs/segment/unify/identity-resolution/identity-resolution-settings
inLanguage: en
dateModified: 2026-06-26T20:05:21.000Z
author:
  "@type": Organization
...
> \[!NOTE]
>
> The steps in this guide pertain to spaces created before September 27th, 2020. For spaces created after September 27, 2020, refer to the [Identity Resolution Onboarding](/docs/segment/unify/identity-resolution/identity-resolution-onboarding/) docs.

## Configure Identity Graph rules

Before you connect a source to Unify, Segment recommends that you first review the default Identity settings and configure custom rules as needed. Segment applies configuration updates to all *new* data flowing through the space after you save your changes. As a result, if this is your first time setting up your Identity Graph, Segment recommends that you get started with a *Dev* space in the [Space Setup](/docs/segment/unify/identity-resolution/space-setup/) docs.
...
> \[!WARNING]
>
> Making a space's Identity Resolution rules less restrictive by changing the [limit](/docs/segment/unify/identity-resolution/identity-resolution-settings/#limit) shouldn't cause any issues to existing or future profiles. <br /><br /> However, making a space's rules more restrictive might affect existing profiles that don't adhere to the new rules (for example, decreasing an identifier's limit or changing the [priority](/docs/segment/unify/identity-resolution/identity-resolution-settings/#priority) of identifiers).
> <br /><br />Segment recommends to get started with a Dev space in the [Space Setup](/docs/segment/unify/identity-resolution/space-setup/) docs, test the rules with the expected data, and then create an identical Production space with those rules. Document any changes to a space's Identity Resolution rules, and don't update rules to be more restrictive after profiles already exist outside the bounds of those new rules.

## Case insensitivity

Segment treats all identifiers as case-insensitive. This means that identifier values such as `user-123`, `USER-123`, and `User-123` are all recognized as the same identity and merged into a single profile.

This behavior applies to all identifier types, including:

* `user_id`
* `anonymous_id`
* `group_id`
* Custom identifiers (email, phone, and more)

When an identifier is first received, Segment stores and uses that original casing as the canonical representation within the profile. Subsequent events with the same identifier in different casings match to the same identity.
...
| braze\_id           | context.Braze.braze\_id or context.Braze.braze\_id when Braze is connected as a destination                   |
| cross\_domain\_id   | cross\_domain\_id when XID is enabled for the workspace                                                       |
...
You'll notice that these identifiers have the *Default* label next to them under *Identifier Type*.
...
![Table showing blocked user\_id values like -1, anonymous, and null with options to add custom or recommended values.](https://docs-resources.prod.twilio.com/2e9b7745d33253be1f9da910ee7c599aa9f935d7187054d89c440ec5d2eba0c5.png)
...
![An example of a Value Limit of 5 anonymous\_ids over a 7-day period.](https://docs-resources.prod.twilio.com/24d2b6ed8cdb54f64680c6c71228971e4613535dbff49600caa8345e1229ae83.png)
...
A profile already exists with `user_id` **abc123** and `email` **[jane@example1.com](mailto:jane@example1.com)**. A new event comes in with new `user_id` **abc456** but the same `email` **[jane@example1.com](mailto:jane@example1.com)**.
...
At this point, the event searches for any profiles that match the identifier user\_id `abc456`. Now there are no existing profiles with this identifier, so Segment creates a new profile with user\_id `abc456`.

By default, Segment explicitly orders user\_id and email as rank `1` and `2`, respectively. All other identifiers are in alphabetical order beginning from rank `3`. This means that if the identifiers sent with events flowing into profiles are user\_id, email, anonymous\_id, and ga\_client\_id, the rank would be as follows:
...
![Identity Resolution Configuration with priority and identifier details.](https://docs-resources.prod.twilio.com/831be5798b13aeb4075b7b391e8b622a6ea96fd8536575b8d2cf60d41e290f68.png)
...
1. Is it an immutable ID? Give immutable IDs, such as user\_id, highest priority.
...
3. Does it temporarily identify a user? Identifiers such as anonymous\_id, ios.idfa, and ga\_client\_id are constantly updated or expired for a user. Generally speaking, rank these lower than identifiers that permanently identify a user.

## Identifier normalization

To prevent confusion and ensure consistent identifier behavior, we recommend normalizing the casing of identifiers before sending events to Segment.

Normalize your identifiers by:

* Choosing a consistent casing convention for each identifier type (for example, always lowercase)
* Applying this normalization at your data source before sending events to Segment
* Documenting your normalization strategy for your team

For example, instead of sending `userId: "USER-123"` in one event and `userId: "user-123"` in another, normalize to a single format such as `userId: "user-123"` for all events.

This prevents duplicate-looking entries in the Profile explorer and reduces confusion for support and analytics teams.
