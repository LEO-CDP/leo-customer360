...
The `customer360-backend/` is separate from `customer360-api/`, which provides the request/response API layer for applications and administrative operations.
...
The local development scripts run Dagster from the `customer360-backend` directory.
...
```text
customer360-backend/.dagster_home
```

Local Dagster logs are stored under:

```text
customer360-backend/logs/
```

The main local log file is:

```text
customer360-backend/logs/dagster.log
```
...
| `DAGSTER_HOME`                       | `customer360-backend/.dagster_home` | Persistent run/event storage for local development  |
...
`customer360-backend/` is the **Dagster orchestration and data-processing layer** for the Customer 360 platform.
...
The long-term objective is to evolve `customer360-backend/` into a reliable, observable, and scalable **Customer 360 CDP processing platform** where customer data continuously flows from ingestion through identity resolution, analytical intelligence, decisioning, activation, and feedback.