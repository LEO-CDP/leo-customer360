...
if [ -z "$PID" ]; then
    echo -e "${YELLOW}No running customer360-backend Dagster process found.${NC}"
    rm -f "$PID_FILE"
else
    ...
    echo "Stopping customer360-backend Dagster (PID $PID, process group)..."
...
# references this checkout's customer360-backend directory (webserver/daemon
