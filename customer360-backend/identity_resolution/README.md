...
This service runs inside `customer360-backend` and is scheduled by Dagster. The default poll interval is 10 minutes.
...
Run the module directly from the `customer360-backend` directory:
...
Start the Dagster code location from `customer360-backend`:
...
| Dagster cannot import the package | Start from `customer360-backend` or use the configured workspace; `dagster_defs.py` adds the identity-resolution directory to `sys.path` for workspace loading. |
