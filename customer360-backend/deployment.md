...
`customer360-backend/`. It defines the supported topology, required state stores,
...
`customer360-backend` is currently deployed as **one Docker image** containing all
...
1. Review changes under `customer360-backend/`, the workspace registration, and all
	dependency files.
...
the entire `customer360-backend/` directory. A running container will not see source
changes on the host. Rebuild and redeploy `customer360-backend` after changes to any
of the following:
...
- `customer360-backend/Dockerfile`.
...
Changes under `customer360-backend/identity_resolution/` rebuild the same unified
...
docker build \
	...
	-f customer360-backend/Dockerfile \
	customer360-backend
```
...
docker run -d \
	...
	--env-file customer360-backend/.env \
...
```bash
docker build -t customer360-dagster:local -f ../customer360-backend/Dockerfile ../customer360-backend
...
`customer360-backend` container with `dagster dev`, uses host networking, and passes
...
It deploys the `customer360-backend` service to the backend VM. In normal CD mode it
...
The CI workflow should treat `customer360-backend/**` as one image build scope:

```text
customer360-backend/**
	-> test all available customer360-backend checks
	-> build customer360-dagster image
```

A customer360-backend change must not be handled as an image-free Dagster config
...
release tag should publish the complete unified customer360-backend image.
...
```bash
sudo docker ps --filter name=customer360-backend
sudo docker logs --tail=200 customer360-backend
...
Until those requirements exist, one `customer360-backend` image is simpler and
