"""Unit tests for dagster_defs (Dagster job/op wrapping the CIR task runner).

Runs the job fully in-process via Dagster's own executor (no real Postgres
needed) with `run_identity_resolution_tasks` mocked out, so these tests
...
class TestIdentityResolutionJob:
    def test_runs_successfully_and_returns_processed_count(self, monkeypatch):
        monkeypatch.setattr(
            dagster_defs, "run_identity_resolution_tasks", MagicMock(return_value=7)
        )
        ...
        dagster_defs.run_identity_resolution_tasks.assert_called_once_with()

    def test_zero_processed_is_still_a_successful_run(self, monkeypatch):
        monkeypatch.setattr(
            dagster_defs, "run_identity_resolution_tasks", MagicMock(return_value=0)
        )
