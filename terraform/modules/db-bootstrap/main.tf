...
#   0. create the dedicated Keycloak database db_keycloak (the app's KC_DB_URL
#      points here; every non-TF path already creates it - keep parity)
...
resource "null_resource" "bootstrap" {
  ...
  triggers = {
    ...
    keycloak_sha = (var.create_keycloak_db && var.keycloak_db_sql != "") ? filesha1(var.keycloak_db_sql) : ""
  }
  ...
  provisioner "local-exec" {
    ...
    command = templatefile("${path.module}/templates/run.sh.tftpl", {
      ...
      create_keycloak_db = var.create_keycloak_db && var.keycloak_db_sql != ""
      keycloak_db_sql    = var.keycloak_db_sql != "" ? abspath(var.keycloak_db_sql) : ""
    })
  }
}
