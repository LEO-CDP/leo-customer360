#!/usr/bin/env bash
# Deploy VNG Cloud vStorage Object Storage buckets to an environment overlay.
# Usage:
...
# Secrets (access_key, secret_key) come from the git-ignored terraform.tfvars
# or .env (TF_VAR_*), shared across envs for now.
#
# State isolation: each env is a separate Terraform WORKSPACE, so uat and prod
# keep independent state and never touch each other's buckets. For runs across
