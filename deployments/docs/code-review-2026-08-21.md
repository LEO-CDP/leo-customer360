...
`server/deploy-api.sh:115` · `server/deploy-backend.sh:71` · `server/seed_data.sh:70` · `cache/deploy.sh:76` · `sso/deploy-sso.sh:89-91` · `customer360-promotions/deploy-ads.sh:118` · `frontend/deploy-frontend.sh:88` · `postgres/run-sql.sh:70-74` · `monitoring/deploy-monitoring.sh:100,137-146`
...
App scripts (`deploy-api.sh:39`, `deploy-backend.sh:33`, `seed_data.sh:42`, `cache/deploy.sh:53`, `sso/deploy-sso.sh:56`, `proxy/deploy-caddy.sh:56`, `frontend/deploy-frontend.sh:46`, `customer360-promotions/deploy-ads.sh:45`, `monitoring/deploy-monitoring.sh:85`) read `floating_ip` **only** from `internal_interfaces`; but `postgres/run-sql.sh:49-62` scans **both** `external_interfaces` and `internal_interfaces`.
...
- **N3 — `server/deploy-api.sh:67`, `customer360-promotions/deploy-ads.sh:71`:** hardcode `terraform workspace select prod` instead of `"$ENV"`. Correct only because `prod` is the sole non-`uat` env today; breaks if another is added.
