...
| [`monitoring`](./monitoring) | Portainer + Netdata dashboards behind oauth2-proxy (Keycloak SSO) — on the api box |
...
flowchart TB
  ...
  lb["L4 Network Load Balancer<br/>103.245.254.29<br/>:80→api · :3000→dagster · :8080→keycloak · :8890→frontend · :9009→ads<br/>:9443→portainer · :19999→netdata (via SSO)"]
  client --> lb
  ...
  subgraph vpc["VPC c360-vpc-uat · subnet 10.100.1.0/24 · HCM03-1C"]
    ...
    subgraph mon["monitoring · on the api box · Keycloak-SSO-gated"]
      oauth2["oauth2-proxy (SSO gate)<br/>:4443 / :4199 → Keycloak"]
      portainer["Portainer<br/>:9443 (loopback)"]
  ...
  ads -->|leo_ads| pg
  dagster -->|SQL| pg
  lb -.->|":9443/:19999 SSO"| oauth2
  oauth2 -->|gates| portainer
...
| oauth2-proxy | api box `10.100.1.5` | 4443 / 4199 | Keycloak SSO gate in front of the dashboards (the L4 LB can't do OIDC) |
| Portainer | api box `10.100.1.5` | 9443 | container ops UI (logs/exec/restart); binds `127.0.0.1` |
| Netdata | api box `10.100.1.5` | 19999 | real-time host + per-container metrics; no native auth (SSO-gated) |
...
| Portainer (SSO) | `http://103.245.254.29:9443` |
...
- **monitoring (Portainer/Netdata) → SSO** — the L4 NLB can't do OIDC, so `oauth2-proxy` fronts both dashboards: `LB :9443/:19999 → oauth2-proxy :4443/:4199 → [Keycloak login] → Portainer/Netdata`. The dashboards read the Docker socket for container discovery; no DB/Redis.
