# deployments/customer360-promotions — c360 Promotions

Deploys `customer360-promotions` (FastAPI/uvicorn, port **9009**) — the ad decisioning API. It
...
`deploy-ads.sh` discovers the target VM from `../server` (by `customer360_promotions_key`), ships
`customer360-promotions/`, **bootstraps the `leo_ads` schema** (`sql-scripts/db-schema-init.sql`,
