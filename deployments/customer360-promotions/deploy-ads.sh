#!/usr/bin/env bash
# Deploy customer360-promotions (c360 Promotions, FastAPI :9009) onto a VM and bootstrap its
...
REPO_ROOT="$(cd ../.. && pwd)" # repo root (contains customer360-promotions/)
...
CUSTOMER360_PROMOTIONS_KEY="${CUSTOMER360_PROMOTIONS_KEY:-$(tfval customer360_promotions_key "$ovl")}"; CUSTOMER360_PROMOTIONS_KEY="${CUSTOMER360_PROMOTIONS_KEY:-api}"
...
c360_PROMOTION_ROOT_PATH="$(tfval ads_root_path "$ovl")"   # e.g. /ads when fronted by Caddy under that path (empty = served at root)
...
FIP="$(printf '%s' "$SERVERS_JSON" | python3 -c 'import json,sys; d=json.load(sys.stdin); s=d.get(sys.argv[1]) or {}; print(next((i.get("floating_ip") for i in (s.get("internal_interfaces") or []) if i.get("floating_ip")), ""))' "$CUSTOMER360_PROMOTIONS_KEY")"
[[ -n "$FIP" ]] || { echo "ERROR: no floating IP for server key '$CUSTOMER360_PROMOTIONS_KEY' — define it in ../server/overlays/$ENV.tfvars."; exit 1; }
BASTION="${BASTION_USER:-leocdp360}@$FIP"
SSH_OPTS=(-i "$SSH_KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null)

if [[ "$ACTION" == "destroy" ]]; then
  echo ">> Removing customer360-promotions container on $BASTION ..."
  ssh "${SSH_OPTS[@]}" "$BASTION" 'sudo docker rm -f customer360-promotions >/dev/null 2>&1; echo "   removed"'
  exit 0
fi
...
if [[ "${BUILD_LOCAL:-0}" == "1" ]]; then
  ...
  echo ">> Shipping customer360-promotions/ ..."
  tar -C "$REPO_ROOT" -czf - customer360-promotions \
  ...
  # The leo_ads schema bootstrap (below) runs psql against customer360-promotions/sql-scripts/
  # on the VM, so ship just those SQL files even when the image comes from GHCR.
  echo ">> Shipping customer360-promotions/sql-scripts/ (schema bootstrap) ..."
  tar -C "$REPO_ROOT" -czf - customer360-promotions/sql-scripts \
    | ssh "${SSH_OPTS[@]}" "$BASTION" 'sudo mkdir -p /opt/c360 && sudo chown "$(id -un)" /opt/c360 && tar -C /opt/c360 -xzf -'
fi
...
OTEL_LINES="$(otel_env_lines customer360-promotions "$ENV" "$JAEGER_HOST")"
...
ENVB64="$(printf '%s' "c360_PROMOTION_ENVIRONMENT=$ENVIRONMENT
c360_PROMOTION_DB_HOST=$DB_HOST
c360_PROMOTION_DB_PORT=$DB_PORT
c360_PROMOTION_DB_NAME=$DB_NAME
c360_PROMOTION_DB_USER=$DB_USER
c360_PROMOTION_DB_PASSWORD=$DB_PASS
c360_PROMOTION_DB_SCHEMA=$DB_SCHEMA
c360_PROMOTION_REDIS_HOST=${REDIS_HOST:-}
c360_PROMOTION_REDIS_PORT=${REDIS_PORT:-6580}
c360_PROMOTION_REDIS_PASSWORD=${REDIS_PASS:-}
c360_PROMOTION_CACHE_ENABLED=$CACHE_ENABLED
c360_PROMOTION_ROOT_PATH=$c360_PROMOTION_ROOT_PATH
...
psql -v ON_ERROR_STOP=1 -h "$DBHOST" -p "$DBPORT" -U "$DBUSER" -d "$DBNAME" -f /opt/c360/customer360-promotions/sql-scripts/db-schema-init.sql >/dev/null
if [ "$SEED" = "true" ]; then
  echo "   loading sample data ..."
  psql -v ON_ERROR_STOP=1 -h "$DBHOST" -p "$DBPORT" -U "$DBUSER" -d "$DBNAME" -f /opt/c360/customer360-promotions/sql-scripts/sample-data-init.sql >/dev/null
fi
...
if [ "$DEPLOY_MODE" = "ghcr" ]; then
  ...
  sed -i 's/ --mount=[^ ]*//g' /opt/c360/customer360-promotions/Dockerfile   # docker.io: no buildx
  sudo docker build -t customer360-promotions /opt/c360/customer360-promotions
  RUN_IMG="customer360-promotions"
fi
sudo docker rm -f customer360-promotions >/dev/null 2>&1 || true
sudo docker run -d --name customer360-promotions --restart unless-stopped --network host --env-file /opt/c360/ads.env "$RUN_IMG"
sleep 3
curl -fsS "http://127.0.0.1:$PORT/health" >/dev/null 2>&1 && echo "   health OK (:$PORT/health)" || echo "   WARN: health not ready yet"
sudo docker ps --filter name=customer360-promotions --format '   running: {{.Names}} ({{.Status}})'
REMOTE
)
