...
tar -C "$REPO_ROOT" -czf - docs \
  | ssh "${SSH_OPTS[@]}" "$BASTION" 'sudo mkdir -p /opt/c360/docs-vector-search && sudo chown "$(id -un)" /opt/c360/docs-vector-search && sudo rm -rf /opt/c360/docs-vector-search/corpus && sudo tar -C /opt/c360/docs-vector-search -xzf - && sudo mv /opt/c360/docs-vector-search/docs /opt/c360/docs-vector-search/corpus'
