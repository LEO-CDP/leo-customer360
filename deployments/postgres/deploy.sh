#!/usr/bin/env bash
# Deploy the GreenNode/VNG Cloud vDB PostgreSQL instance to an environment overlay.
# Usage:
...
# Secrets (client_id/secret, db_password) come from the git-ignored
# terraform.tfvars or .env (TF_VAR_*), shared across envs for now.
#
# State isolation: each env is a separate Terraform WORKSPACE, so uat and prod
# keep independent state and never touch each other's instance. For runs across
