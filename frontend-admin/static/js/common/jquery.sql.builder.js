...
QueryBuilder.prototype.updateGroupCondition = function(group, previousCondition) {
    group.$el.find('>' + QueryBuilder.selectors.group_condition).each(function() {
        ...
        $this.parent().toggleClass('active bg-blue-600 text-white', $this.val() === group.condition);
        $this.parent().toggleClass('bg-white text-gray-700', $this.val() !== group.condition);
    });
    ...
    (function walk(group) {
        if (!group.flags || (group.flags && !group.flags.condition_readonly)) {
            group.$el.find('>' + QueryBuilder.selectors.group_condition).prop('disabled', group.rules.length <= 1)
                .parent().toggleClass('opacity-50 cursor-not-allowed', group.rules.length <= 1);
        }
...
QueryBuilder.prototype.createRuleInput = function(rule) {
    ...
    for (var i = 0; i < rule.operator.nb_inputs; i++) {
        ...
        if (i > 0) $valueContainer.append(`<span class="mx-2">${this.settings.inputs_separator}</span>`);
...
QueryBuilder.prototype.applyGroupFlags = function(group) {
    ...
    group.$el.find('>' + Selectors.group_condition).prop('disabled', flags.condition_readonly)
        .parent().toggleClass('opacity-50 cursor-not-allowed', flags.condition_readonly);
...
QueryBuilder.prototype.updateError = function(node) {
    if (this.settings.display_errors) {
        if (node.error === null) {
            node.$el.removeClass('has-error border-red-500');
            node.$el.find(QueryBuilder.selectors.error_container).eq(0).removeClass('block').addClass('hidden');
        }
        else {
            ...
            node.$el.addClass('has-error border-red-500')
                .find(QueryBuilder.selectors.error_container).eq(0).removeClass('hidden').addClass('block')
...
// ============================================================================
// TEMPLATES (Updated for Tailwind CSS)
// ============================================================================

QueryBuilder.templates.group = ({ group_id, level, conditions, icons, settings, translate, builder }) => {
  return `
<div id="${group_id}" class="rules-group-container p-4 border border-blue-200 bg-blue-50/50 rounded-lg mb-4 shadow-sm">
  <div class="rules-group-header flex flex-wrap justify-between items-center mb-3">
    <div class="inline-flex rounded-md shadow-sm group-conditions">
      ${conditions.map((condition, i) => `
        <label class="cursor-pointer inline-flex items-center justify-center px-3 py-1.5 text-sm font-medium border border-blue-600 bg-white text-blue-600 hover:bg-blue-50 transition-colors ${i === 0 ? 'rounded-l-md' : ''} ${i === conditions.length - 1 ? 'rounded-r-md' : ''} [&:not(:first-child)]:-ml-px">
          <input type="radio" class="sr-only" name="${group_id}_cond" value="${condition}"> ${translate("conditions", condition)}
        </label>
      `).join('\n')}
    </div>
    
    <div class="flex space-x-2 ml-auto group-actions">
      <button type="button" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-green-500" data-add="rule">
        <i class="${icons.add_rule} mr-1"></i> ${translate("add_rule")}
      </button>
      ${settings.allow_groups === -1 || settings.allow_groups >= level ? `
        <button type="button" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-green-500" data-add="group">
          <i class="${icons.add_group} mr-1"></i> ${translate("add_group")}
        </button>
      ` : ''}
      ${level > 1 ? `
        <button type="button" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-red-500" data-delete="group">
          <i class="${icons.remove_group} mr-1"></i> ${translate("delete_group")}
        </button>
      ` : ''}
    </div>
    ${settings.display_errors ? `
      <div class="error-container hidden text-red-600 ml-3 text-lg cursor-help"><i class="${icons.error}"></i></div>
    ` : ''}
  </div>
  <div class="rules-group-body">
    <div class="rules-list flex flex-col space-y-3"></div>
  </div>
</div>`;
};
...
QueryBuilder.templates.rule = ({ rule_id, icons, settings, translate, builder }) => {
  return `
<div id="${rule_id}" class="rule-container flex flex-wrap items-center gap-3 p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
  <div class="rule-header flex-shrink-0 w-full flex justify-between items-center md:w-auto md:order-last md:ml-auto">
    ${settings.display_errors ? `
      <div class="error-container hidden text-red-600 mr-3 text-lg cursor-help"><i class="${icons.error}"></i></div>
    ` : ''}
    <div class="inline-flex rule-actions">
      <button type="button" class="inline-flex items-center px-2 py-1.5 text-sm font-medium rounded-md text-red-600 bg-red-50 hover:bg-red-100 hover:text-red-700 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500" data-delete="rule">
        <i class="${icons.remove_rule}"></i> <span class="ml-1 hidden md:inline">${translate("delete_rule")}</span>
      </button>
    </div>
  </div>
  <div class="rule-filter-container flex-1 min-w-[150px]"></div>
  <div class="rule-operator-container flex-1 min-w-[150px]"></div>
  <div class="rule-value-container flex-1 min-w-[200px] flex flex-wrap items-center gap-2"></div>
</div>`;
};
...
QueryBuilder.templates.filterSelect = ({ rule, filters, icons, settings, translate, builder }) => {
  let optgroup = null;
  return `
<select class="block w-full px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" name="${rule.id}_filter">
...
QueryBuilder.templates.operatorSelect = ({ rule, operators, icons, settings, translate, builder }) => {
  ...
  return `
${operators.length === 1 ? `
<span class="inline-block px-3 py-1.5 text-sm text-gray-700 bg-gray-50 border border-gray-200 rounded-md w-full">
${translate("operators", operators[0].type)}
</span>
` : ''}
<select class="block w-full px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${operators.length === 1 ? 'hidden' : ''}" name="${rule.id}_operator">
...
QueryBuilder.templates.ruleValueSelect = ({ name, rule, icons, settings, translate, builder }) => {
  let optgroup = null;
  return `
<select class="block w-full px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" name="${name}" ${rule.filter.multiple ? 'multiple' : ''}>
...
QueryBuilder.prototype.getRuleInput = function (rule, value_id) {
  ...
  var c = filter.vertical ? ' class="block mb-2"' : ' class="inline-flex items-center mr-4"';
  var h = '';
  var placeholder = Array.isArray(filter.placeholder) ? filter.placeholder[value_id] : filter.placeholder;
  var commonInputClass = 'block w-full px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500';
    ...
    switch (filter.input) {
      ...
      case 'checkbox':
        Utils.iterateOptions(filter.values, function (key, val) {
          h += '<label' + c + '><input class="text-blue-600 focus:ring-blue-500 border-gray-300 ' + (filter.input === 'radio' ? 'rounded-full' : 'rounded') + '" type="' + filter.input + '" name="' + name + '" value="' + key + '"> <span class="ml-2 text-sm text-gray-900">' + val + '</span></label> ';
        });
        break;
      ...
      case 'textarea':
        h += '<textarea class="' + commonInputClass + '" name="' + name + '"';
      ...
      case 'number':
        h += '<input class="' + commonInputClass + '" type="number" name="' + name + '"';
...
/**
 * Iterates over radio/checkbox/selection options
...
/**
 * Sorts objects by grouping them by `key`
 * @param {object[]} items
 * @param {string} key
 * @returns {object[]}
 */
 ...
 * @description Applies Tailwind CSS styling for checkbox and radio inputs.
 */
QueryBuilder.define('tw-checkbox', function(options) {
    this.on('getRuleInput.filter', function(h, rule, name) {
        var filter = rule.filter;

        if ((filter.input === 'radio' || filter.input === 'checkbox') && !filter.plugin) {
            h.value = '';
            var style = filter.vertical ? 'block mb-2' : 'inline-flex items-center mr-4 mb-2';
            var i = 0;

            Utils.iterateOptions(filter.values, function(key, val) {
                var id = name + '_' + (i++);
                var roundedClass = filter.input === 'radio' ? 'rounded-full' : 'rounded';
                
                h.value += `<div class="${style}"> 
                  <input class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 ${roundedClass} shadow-sm" type="${filter.input}" name="${name}" id="${id}" value="${key}"> 
                  <label class="ml-2 block text-sm text-gray-900 cursor-pointer" for="${id}">${val}</label>
                </div>`;
            });
        }
    });
...
/**
 * @class TwTooltipErrors
 * @memberof module:plugins
 * @description Uses native titles for simple, zero-dependency validation error tooltip behaviors.
 */
QueryBuilder.define('tw-tooltip-errors', function(options) {
    var self = this;

    // Use native title instead of Bootstrap's tooltip structure
    this.model.on('update', function(e, node, field) {
        if (field == 'error' && self.settings.display_errors) {
            var errorMessage = node.error ? self.translate('errors', node.error[0]) : '';
            if(node.error && node.error.length > 1) {
                errorMessage = Utils.fmt(errorMessage, node.error.slice(1));
            }
...
QueryBuilder.define('chosen-selectpicker', function(options) {
    ...
    this.on('afterCreateRuleFilters', function(e, rule) {
        rule.$el.find(Selectors.rule_filter).chosen(options);
    });

    this.on('afterCreateRuleOperators', function(e, rule) {
        if (e.builder.getOperators(rule.filter).length > 1) {
            rule.$el.find(Selectors.rule_operator).chosen(options);
        }
    });
 ...
 * @memberof module:plugins
 * @description Provides a description about a filter (Tailwind adapted)
 */
    ...
    if (options.mode === 'inline') {
        this.on('afterUpdateRuleFilter afterUpdateRuleOperator', function(e, rule) {
            ...
            if (!description) {
                ...
                if ($p.length === 0) {
                    $p = $($.parseHTML('<p class="filter-description text-sm text-gray-500 mt-1 w-full"></p>'));
                    $p.appendTo(rule.$el);
                }
                else {
                    $p.css('display', '');
                }
                $p.html('<i class="' + options.icon + ' mr-1"></i> ' + description);
            }
        });
    }
    // Simple native tooltip replaces bootstrap popover/bootbox
    else {
        this.on('afterUpdateRuleFilter afterUpdateRuleOperator', function(e, rule) {
            ...
            if (!description) {
                ...
                if ($b.length === 0) {
                    $b = $($.parseHTML('<button type="button" class="inline-flex items-center justify-center p-1.5 text-sm font-medium rounded-full text-blue-600 bg-blue-50 hover:bg-blue-100 ml-2 filter-description focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors" title=""><i class="' + options.icon + '"></i></button>'));
                    $b.prependTo(rule.$el.find(QueryBuilder.selectors.rule_actions));
                }
                else {
                    $b.css('display', '');
                }
                $b.attr('title', description);
            }
        });
    }
}, {
    icon: 'bi-info-circle-fill',
    mode: 'popover' // Replaces popover internally with standard native title attribute
});
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            ...
            $h.find(Selectors.condition_container).after(
                '<button type="button" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 ml-2 shadow-sm transition-colors" data-invert="group">' +
                '<i class="' + options.icon + ' mr-1"></i> ' + self.translate('invert') +
                '</button>'
            );
        ...
        if (options.display_rules_button && options.invert_rules) {
            this.on('getRuleTemplate.filter', function(h) {
                ...
                $h.find(Selectors.rule_actions).prepend(
                    '<button type="button" class="inline-flex items-center px-2 py-1.5 text-sm font-medium rounded-md text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 shadow-sm transition-colors mr-2" data-invert="rule">' +
                    '<i class="' + options.icon + '"></i><span class="ml-1 hidden md:inline">' + self.translate('invert') + '</span>' +
                    '</button>'
                );
                h.value = $h.prop('outerHTML');
            });
...
QueryBuilder.defaults({
    ...
    mongoRuleOperators: {
        $eq: function(v) { return { 'val': v, 'op': v === null ? 'is_null' : (v === '' ? 'is_empty' : 'equal') }; },
        $ne: function(v) { v = v.$ne; return { 'val': v, 'op': v === null ? 'is_not_null' : (v === '' ? 'is_not_empty' : 'not_equal') }; },
        $regex: function(v) {
            v = v.$regex;
            if (v.slice(0, 4) == '^(?!' && v.slice(-1) == ')') { return { 'val': v.slice(4, -1), 'op': 'not_begins_with' }; }
...
QueryBuilder.extend({
    getMongo: function(data) {
        ...
        return (function parse(group) {
            if (!group.condition) group.condition = self.settings.default_condition;
            ...
            group.rules.forEach(function(rule) {
                if (rule.rules && rule.rules.length > 0) { parts.push(parse(rule)); }
                else {
                    var mdb = self.settings.mongoOperators[rule.operator];
                    var ope = self.getOperatorByType(rule.operator);
                    if (mdb === undefined) Utils.error('UndefinedMongoOperator', 'Unknown MongoDB operation for operator "{0}"', rule.operator);
                    if (ope.nb_inputs !== 0 && !(rule.value instanceof Array)) rule.value = [rule.value];
    ...
    getRulesFromMongo: function(query) {
        ...
        if ('rules' in query && 'condition' in query) return query;
        ...
        if (!key) Utils.error('MongoParse', 'Invalid MongoDB query format');
        return (function parse(data, topKey) {
            var rules = data[topKey];
            var parts = [];
            rules.forEach(function(data) {
                data = self.change('parseMongoNode', data);
                if ('rules' in data && 'condition' in data) { parts.push(data); return; }
                if ('id' in data && 'operator' in data && 'value' in data) { parts.push(data); return; }
                var key = self.getMongoCondition(data);
                if (key) { parts.push(parse(data, key)); }
                else {
                    var field = Object.keys(data)[0];
                    var value = data[field];
                    var operator = self.getMongoOperator(value);
                    if (operator === undefined) Utils.error('MongoParse', 'Invalid MongoDB query format');
                    var mdbrl = self.settings.mongoRuleOperators[operator];
                    if (mdbrl === undefined) Utils.error('UndefinedMongoOperator', 'JSON Rule operation unknown for operator "{0}"', operator);
                    var opVal = mdbrl.call(self, value);
                    var id = self.getMongoDBFieldID(field, value);
                    var rule = self.change('mongoToRule', { id: id, field: field, operator: opVal.op, value: opVal.val }, data);
                    parts.push(rule);
                }
            });
            return self.change('mongoToGroup', { condition: topKey.replace('$', '').toUpperCase(), rules: parts }, data);
        }(query, key));
    },
    setRulesFromMongo: function(query) { this.setRules(this.getRulesFromMongo(query)); },
    getMongoDBFieldID: function(field, value) {
        var matchingFilters = this.filters.filter(function(filter) { return filter.field === field; });
        var id;
        if (matchingFilters.length === 1) id = matchingFilters[0].id;
        else id = this.change('getMongoDBFieldID', field, value);
        return id;
    },
    getMongoOperator: function(data) {
        if (data !== null && typeof data === 'object') {
            if (data.$gte !== undefined && data.$lte !== undefined) return 'between';
            if (data.$lt !== undefined && data.$gt !== undefined) return 'not_between';
            var knownKeys = Object.keys(data).filter(function(key) { return !!this.settings.mongoRuleOperators[key]; }.bind(this));
    ...
    getMongoCondition: function(data) {
        ...
        for (var i = 0, l = keys.length; i < l; i++) {
            if (keys[i].toLowerCase() === '$or' || keys[i].toLowerCase() === '$and') return keys[i];
        }
    }
});
...
QueryBuilder.define('not-group', function(options) {
    ...
    this.on('afterInit', function() {
        ...
        self.model.on('update', function(e, node, field) {
            if (node instanceof Group && field === 'not') self.updateGroupNot(node);
        });
    });
    this.on('afterAddGroup', function(e, group) { group.__.not = false; });

    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            var $h = $($.parseHTML(h.value));
            $h.find(QueryBuilder.selectors.condition_container).prepend(
                '<button type="button" class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 mr-2 shadow-sm transition-colors" data-not="group">' +
                '<i class="' + options.icon_unchecked + ' mr-1"></i> ' + self.translate('NOT') +
                '</button>'
            );
            h.value = $h.prop('outerHTML');
        });
    ...
    this.on('groupToJson.filter', function(e, group) { e.value.not = group.not; });
    this.on('jsonToGroup.filter', function(e, json) { e.value.not = !!json.not; });
    this.on('groupToSQL.filter', function(e, group) { if (group.not) e.value = 'NOT ( ' + e.value + ' )'; });
    this.on('parseSQLNode.filter', function(e) {
        if (e.value.name && e.value.name.toUpperCase() == 'NOT') {
            ...
            if (['AND', 'OR'].indexOf(e.value.operation.toUpperCase()) === -1) {
                e.value = new SQLParser.nodes.Op(self.settings.default_condition, e.value, null);
            }
            e.value.not = true;
        }
    });
    this.on('sqlGroupsDistinct.filter', function(e, group, data, i) { if (data.not && i > 0) e.value = true; });
    this.on('sqlToGroup.filter', function(e, data) { e.value.not = !!data.not; });
    this.on('groupToMongo.filter', function(e, group) {
        var key = '$' + group.condition.toLowerCase();
        if (group.not && e.value[key]) e.value = { '$nor': [e.value] };
    });
    ...
    this.on('mongoToGroup.filter', function(e, data) { e.value.not = !!data.not; });
}, {
...
QueryBuilder.extend({
    updateGroupNot: function(group) {
        ...
        group.$el.find('>' + QueryBuilder.selectors.group_not)
            .toggleClass('bg-red-600 text-white border-red-600 hover:bg-red-700', group.not)
            .toggleClass('bg-white text-gray-700 hover:bg-gray-50', !group.not)
            .find('i').attr('class', group.not ? options.icon_checked + ' mr-1' : options.icon_unchecked + ' mr-1');
...
QueryBuilder.define('sortable', function(options) {
    if (!('interact' in window)) Utils.error('MissingLibrary', 'interact.js is required to use "sortable" plugin. Get it here: http://interactjs.io');
    ...
    var placeholder; var ghost; var src; var moved;

    this.on('afterAddRule afterAddGroup', function(e, node) {
        if (node == placeholder) return;
        var self = e.builder;
        if (options.inherit_no_sortable && node.parent && node.parent.flags.no_sortable) node.flags.no_sortable = true;
                ...
                .draggable({
                    ...
                    onstart: function(event) {
                        ...
                        ghost = src.$el.clone()
                            ...
                            .addClass('opacity-50 pointer-events-none fixed z-50');
                        var ph = $($.parseHTML('<div class="rule-placeholder h-16 border-2 border-dashed border-gray-400 bg-gray-50 rounded-lg mb-2"></div>'));
                    ...
                    onend: function(event) {
                        if (event.dropzone) { moveSortableToTarget(src, $(event.relatedTarget), self); moved = true; }
                        ghost.remove(); ghost = undefined;
                        placeholder.drop(); placeholder = undefined;
                        src.$el.css('display', '');
        ...
        if (!node.flags.no_drop) {
            interact(node.$el[0])
                .dropzone({
                    ...
                    ondragenter: function(event) { moveSortableToTarget(placeholder, $(event.target), self); },
                    ondrop: function(event) { if (!moved) moveSortableToTarget(src, $(event.target), self); }
                });
            if (node instanceof Group) {
    ...
    this.on('beforeDeleteRule beforeDeleteGroup', function(e, node) {
        if (!e.isDefaultPrevented()) {
            ...
            if (node instanceof Group) interact(node.$el.find(QueryBuilder.selectors.group_header)[0]).unset();
        }
    });
    this.on('afterApplyRuleFlags afterApplyGroupFlags', function(e, node) {
        if (node.flags.no_sortable) node.$el.find('.drag-handle').remove();
    });
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h, level) {
            if (level > 1) {
                var $h = $($.parseHTML(h.value));
                $h.find(QueryBuilder.selectors.condition_container).after('<div class="drag-handle cursor-move px-2 text-gray-500 hover:text-gray-700"><i class="' + options.icon + ' text-lg"></i></div>');
                h.value = $h.prop('outerHTML');
            }
        });
        this.on('getRuleTemplate.filter', function(h) {
            var $h = $($.parseHTML(h.value));
            $h.find(QueryBuilder.selectors.rule_header).after('<div class="drag-handle cursor-move px-2 text-gray-500 hover:text-gray-700 flex items-center justify-center h-full"><i class="' + options.icon + ' text-lg"></i></div>');
            h.value = $h.prop('outerHTML');
        });
    }
}, {
...
QueryBuilder.defaults({
    default_rule_flags: { no_sortable: false, no_drop: false },
    default_group_flags: { no_sortable: false, no_drop: false }
});
...
function moveSortableToTarget(node, target, builder) {
    ...
    if (parent.length) method = 'moveAfter';
    if (!method) {
        parent = target.closest(Selectors.group_header);
        if (parent.length) { parent = target.closest(Selectors.group_container); method = 'moveAtBegin'; }
    }
    if (!method) {
        parent = target.closest(Selectors.group_container);
        if (parent.length) method = 'moveAtEnd';
    }
    if (method) {
        node[method](builder.getModel(parent));
        if (builder && node instanceof Rule) builder.setRuleInputValue(node, node.value);
    }
}
 ...
 * @memberof module:plugins
 */
QueryBuilder.define('sql-support', function(options) {}, { boolean_as_integer: true });
QueryBuilder.defaults({
    sqlOperators: {
        equal: { op: '= ?' }, not_equal: { op: '!= ?' }, in: { op: 'IN(?)', sep: ', ' }, not_in: { op: 'NOT IN(?)', sep: ', ' },
        less: { op: '< ?' }, less_or_equal: { op: '<= ?' }, greater: { op: '> ?' }, greater_or_equal: { op: '>= ?' },
        between: { op: 'BETWEEN ?', sep: ' AND ' }, not_between: { op: 'NOT BETWEEN ?', sep: ' AND ' },
        begins_with: { op: 'LIKE ?', mod: '{0}%', escape: '%_' }, not_begins_with: { op: 'NOT LIKE ?', mod: '{0}%', escape: '%_' },
        contains: { op: 'LIKE ?', mod: '%{0}%', escape: '%_' }, not_contains: { op: 'NOT LIKE ?', mod: '%{0}%', escape: '%_' },
        ends_with: { op: 'LIKE ?', mod: '%{0}', escape: '%_' }, not_ends_with: { op: 'NOT LIKE ?', mod: '%{0}', escape: '%_' },
        is_empty: { op: '= \'\'' }, is_not_empty: { op: '!= \'\'' }, is_null: { op: 'IS NULL' }, is_not_null: { op: 'IS NOT NULL' }
    },
    sqlRuleOperator: {
        '=': function(v) { return { val: v, op: v === '' ? 'is_empty' : 'equal' }; },
        '!=': function(v) { return { val: v, op: v === '' ? 'is_not_empty' : 'not_equal' }; },
        'LIKE': function(v) {
            if (v.slice(0, 1) == '%' && v.slice(-1) == '%') return { val: v.slice(1, -1), op: 'contains' };
            else if (v.slice(0, 1) == '%') return { val: v.slice(1), op: 'ends_with' };
            else if (v.slice(-1) == '%') return { val: v.slice(0, -1), op: 'begins_with' };
            else Utils.error('SQLParse', 'Invalid value for LIKE operator "{0}"', v);
        },
        'NOT LIKE': function(v) {
            if (v.slice(0, 1) == '%' && v.slice(-1) == '%') return { val: v.slice(1, -1), op: 'not_contains' };
            else if (v.slice(0, 1) == '%') return { val: v.slice(1), op: 'not_ends_with' };
            else if (v.slice(-1) == '%') return { val: v.slice(0, -1), op: 'not_begins_with' };
            else Utils.error('SQLParse', 'Invalid value for NOT LIKE operator "{0}"', v);
        },
        'IN': function(v) { return { val: v, op: 'in' }; }, 'NOT IN': function(v) { return { val: v, op: 'not_in' }; },
        '<': function(v) { return { val: v, op: 'less' }; }, '<=': function(v) { return { val: v, op: 'less_or_equal' }; },
        '>': function(v) { return { val: v, op: 'greater' }; }, '>=': function(v) { return { val: v, op: 'greater_or_equal' }; },
        'BETWEEN': function(v) { return { val: v, op: 'between' }; }, 'NOT BETWEEN': function(v) { return { val: v, op: 'not_between' }; },
        'IS': function(v) { if (v !== null) Utils.error('SQLParse', 'Invalid value for IS operator'); return { val: null, op: 'is_null' }; },
        'IS NOT': function(v) { if (v !== null) Utils.error('SQLParse', 'Invalid value for IS operator'); return { val: null, op: 'is_not_null' }; }
    },
    sqlStatements: {
        'question_mark': function() {
            var params = [];
            return { add: function(rule, value) { params.push(value); return '?'; }, run: function() { return params; } };
        },
        'numbered': function(char) {
            if (!char || char.length > 1) char = '$';
            var index = 0; var params = [];
            return { add: function(rule, value) { params.push(value); index++; return char + index; }, run: function() { return params; } };
        },
        'named': function(char) {
            if (!char || char.length > 1) char = ':';
            var indexes = {}; var params = {};
            return { add: function(rule, value) { if (!indexes[rule.field]) indexes[rule.field] = 1; var key = rule.field + '_' + (indexes[rule.field]++); params[key] = value; return char + key; }, run: function() { return params; } };
        }
    },
    sqlRuleStatement: {
        'question_mark': function(values) {
            var index = 0;
            return { parse: function(v) { return v == '?' ? values[index++] : v; }, esc: function(sql) { return sql.replace(/\?/g, '\'?\''); } };
        },
        'numbered': function(values, char) {
            if (!char || char.length > 1) char = '$';
            var regex1 = new RegExp('^\\' + char + '[0-9]+$'); var regex2 = new RegExp('\\' + char + '([0-9]+)', 'g');
            return { parse: function(v) { return regex1.test(v) ? values[v.slice(1) - 1] : v; }, esc: function(sql) { return sql.replace(regex2, '\'' + (char == '$' ? '$$' : char) + '$1\''); } };
        },
        'named': function(values, char) {
            if (!char || char.length > 1) char = ':';
            var regex1 = new RegExp('^\\' + char); var regex2 = new RegExp('\\' + char + '(' + Object.keys(values).join('|') + ')\\b', 'g');
            return { parse: function(v) { return regex1.test(v) ? values[v.slice(1)] : v; }, esc: function(sql) { return sql.replace(regex2, '\'' + (char == '$' ? '$$' : char) + '$1\''); } };
        }
    }
});
...
QueryBuilder.extend({
    getSQL: function(stmt, nl, data) {
        ...
        var sql = (function parse(group) {
            ...
            group.rules.forEach(function(rule) {
                if (rule.rules && rule.rules.length > 0) { parts.push('(' + nl + parse(rule) + nl + ')' + nl); }
                else {
                    var sql = self.settings.sqlOperators[rule.operator];
                    var ope = self.getOperatorByType(rule.operator);
                    var value = '';
                    if (sql === undefined) Utils.error('UndefinedSQLOperator', 'Unknown SQL operation for operator "{0}"', rule.operator);

                    if (ope.nb_inputs !== 0) {
                        if (!(rule.value instanceof Array)) rule.value = [rule.value];
                        rule.value.forEach(function(v, i) {
                            if (i > 0) value += sql.sep;
                            ...
                            else { if (typeof v == 'string') v = '\'' + v + '\''; value += v; }
                        });
                    }
                    var sqlFn = function(v) { return sql.op.replace('?', function() { return v; }); };
        ...
        if (stmt) return { sql: sql, params: stmt.run() };
        else return { sql: sql };
    },
    getRulesFromSQL: function(query, stmt) {
        if (!('SQLParser' in window)) Utils.error('MissingLibrary', 'SQLParser is required to parse SQL queries. Get it here https://github.com/mistic100/sql-parser');
        var self = this;
        if (typeof query == 'string') query = { sql: query };
        if (stmt === true) stmt = 'question_mark';
        if (typeof stmt == 'string') { var config = getStmtConfig(stmt); stmt = this.settings.sqlRuleStatement[config[1]](query.params, config[2]); }
        ...
        if (!parsed.where) Utils.error('SQLParse', 'No WHERE clause found');
        var data = self.change('parseSQLNode', parsed.where.conditions);

        if ('rules' in data && 'condition' in data) return data;
        if ('id' in data && 'operator' in data && 'value' in data) return { condition: this.settings.default_condition, rules: [data] };

        var out = self.change('sqlToGroup', { condition: this.settings.default_condition, rules: [] }, data);
        var curr = out;

        (function flatten(data, i) {
            if (data === null) return;
            data = self.change('parseSQLNode', data);
            if ('rules' in data && 'condition' in data) { curr.rules.push(data); return; }
            ...
            if (['AND', 'OR'].indexOf(data.operation.toUpperCase()) !== -1) {
                ...
                if (createGroup) {
                    var group = self.change('sqlToGroup', { condition: self.settings.default_condition, rules: [] }, data);
                    curr.rules.push(group); curr = group;
                }
                curr.condition = data.operation.toUpperCase(); i++;
                var next = curr;
                flatten(data.left, i);
                curr = next;
                ...
                if ($.isPlainObject(data.right.value)) Utils.error('SQLParse', 'Value format not supported for {0}.', data.left.value);
                var value;
                if ($.isArray(data.right.value)) value = data.right.value.map(function(v) { return v.value; });
                else value = data.right.value;
                if (stmt) {
                    if ($.isArray(value)) value = value.map(stmt.parse);
                ...
                if (sqlrl === undefined) Utils.error('UndefinedSQLOperator', 'Invalid SQL operation "{0}".', data.operation);
                var opVal = sqlrl.call(this, value, data.operation);

                var field;
                if ('values' in data.left) field = data.left.values.join('.');
                ...
                if (!stmt && sql && sql.escape) {
                    var searchChars = sql.escape.split('').map(function(c) { return '\\\\' + c; }).join('|');
                    finalValue = finalValue.replace(new RegExp('(' + searchChars + ')', 'g'), function(s) { return s[1]; });
                }
                var id = self.getSQLFieldID(field, value);
                var rule = self.change('sqlToRule', { id: id, field: field, operator: opVal.op, value: finalValue }, data);
                curr.rules.push(rule);
            }
        }(data, 0));
        return out;
    },
    setRulesFromSQL: function(query, stmt) { this.setRules(this.getRulesFromSQL(query, stmt)); },
    getSQLFieldID: function(field, value) {
        var matchingFilters = this.filters.filter(function(filter) { return filter.field.toLowerCase() === field.toLowerCase(); });
        var id;
        if (matchingFilters.length === 1) id = matchingFilters[0].id;
        ...
        (function walk(group) {
            group.each(function(rule) {
                if (rule.filter && rule.filter.unique) {
                    if (!self.status.used_filters[rule.filter.id]) self.status.used_filters[rule.filter.id] = [];
                    if (rule.filter.unique == 'group') self.status.used_filters[rule.filter.id].push(rule.parent);
                }
            }, function(group) { walk(group); });
        }(self.model.root));
