...
CREATE TABLE IF NOT EXISTS customer360.crm_campaign (
    ...
    CONSTRAINT chk_crm_campaign_approval_status
...
CREATE TABLE IF NOT EXISTS customer360.cdp_master_profiles (
    ...
    -- The foreign-key relationship is maintained by the persona resolution
    -- workflow because cdp_customer_personas also references this table.
...
-- CREATE TABLE IF NOT EXISTS keeps this definition idempotent for fresh
...
-- Constraints are guarded so this section is safe to re-run against an
-- already-migrated database (run-sql.sh re-applies the schema).
