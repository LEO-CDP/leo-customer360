...
if [ -z "$PID" ]; then
    echo -e "${YELLOW}[API] No running LEO AD SERVER API process found.${NC}"
    rm -f "$PID_FILE"
    exit 0
fi
...
echo "[API] Stopping LEO AD SERVER API | PID ${PID}"
...
if kill -0 "$PID" 2>/dev/null; then
    echo -e "${RED}[API] Failed to stop LEO AD SERVER API${NC} | PID ${PID} is still running"
    exit 1
fi

echo -e "${GREEN}[API] LEO AD SERVER API stopped successfully${NC} | PID ${PID}"