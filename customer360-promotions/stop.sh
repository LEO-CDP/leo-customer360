...
if [ -z "$PID" ]; then
    echo -e "${YELLOW}[API] No running c360 PROMOTION API API process found.${NC}"
    rm -f "$PID_FILE"
    exit 0
fi
...
echo "[API] Stopping c360 PROMOTION API API | PID ${PID}"
...
if kill -0 "$PID" 2>/dev/null; then
    echo -e "${RED}[API] Failed to stop c360 PROMOTION API API${NC} | PID ${PID} is still running"
    exit 1
fi

echo -e "${GREEN}[API] c360 PROMOTION API API stopped successfully${NC} | PID ${PID}"