...
High-performance, multi-tenant marketing promotions API built on **FastAPI + SQLAlchemy + PostgreSQL 16**.

Designed for:
- **Standard digital ad banner delivery**, affiliate marketing link tracking, and sponsored content in native ads
- **Real-time content and product recommendations** for websites, mobile applications, and AI agents through the MCP protocol
- **Multi-tenant isolation** with strict `tenant_id` filtering on all operations
- **Production-grade architecture** with clean separation of concerns across repositories, services, and controllers
- **Scalable promotion content** spanning display, native, video, carousel, affiliate, and sponsored formats

## Overview

The c360 Promotions API exposes:
1. **Health endpoints** for monitoring service and database availability
2. **Promotion and campaign endpoints** for retrieving sponsored content and delivery candidates by placement
3. **Placement endpoints** for inventory configuration lookups
4. **Affiliate link tracking** for measurable partner and campaign conversions
5. **Recommendation endpoints** for real-time website, mobile, and MCP-based AI-agent experiences
6. **JavaScript widget** (`ads.loader.js`) for client-side banner and sponsored-content rendering
...
- Ready for Redis caching on hot paths (placement lookups, promotion candidates, and recommendations)
...
cp .env.example .env
# Edit .env:
#   c360_PROMOTION_API_HOST=localhost
#   c360_PROMOTION_API_PORT=9009
#   c360_PROMOTION_DB_HOST=localhost
#   c360_PROMOTION_DB_PORT=5432
#   c360_PROMOTION_DB_USER=postgres
#   c360_PROMOTION_DB_PASSWORD=your_password
#   c360_PROMOTION_DB_NAME=customer360
```
...
```bash
./start.sh --seed-demo-customer360-promotions
```
...
The promotions API is built on four core entities: **Placement**, **Campaign**, **Creative**, and **Ad**.
...
├── start.sh                     # Startup script (with optional --seed-demo-customer360-promotions)
...
- **app.py**: Entry point. Bootstraps `PromotionsApplication` and exposes `app` object for uvicorn
...
c360_PROMOTION_DB_PASSWORD=your_password
c360_PROMOTION_DB_NAME=customer360
c360_PROMOTION_DB_SCHEMA=leo_ads
c360_PROMOTION_DB_POOL_SIZE=10
c360_PROMOTION_DB_MAX_OVERFLOW=20
c360_PROMOTION_DB_ECHO_SQL=false  # Set to true for SQL debugging
```
...
The promotions API follows **clean layering** to keep code maintainable and testable:
...
|----------|---------|---------|
| `c360_PROMOTION_API_HOST` | `localhost` | API listen address |
| `c360_PROMOTION_API_PORT` | `9009` | API listen port |
| `c360_PROMOTION_DB_HOST` | `localhost` | PostgreSQL host |
| `c360_PROMOTION_DB_PORT` | `5432` | PostgreSQL port |
| `c360_PROMOTION_DB_USER` | `postgres` | PostgreSQL user |
| `c360_PROMOTION_DB_PASSWORD` | `password` | PostgreSQL password |
| `c360_PROMOTION_DB_NAME` | `customer360` | Database name |
| `c360_PROMOTION_DB_SCHEMA` | `leo_ads` | Schema name |
| `c360_PROMOTION_DB_POOL_SIZE` | `10` | Connection pool size |
| `c360_PROMOTION_DB_MAX_OVERFLOW` | `20` | Max overflow connections |
| `c360_PROMOTION_DB_POOL_RECYCLE_SECONDS` | `1800` | Connection recycle interval |
| `c360_PROMOTION_DB_ECHO_SQL` | `false` | Enable SQL logging (debug) |
| `c360_PROMOTION_UVICORN_RELOAD` | `false` | Enable auto-reload (dev only) |
| `c360_PROMOTION_CACHE_ENABLED` | `true` | Enable Redis caching |
| `c360_PROMOTION_REDIS_HOST` | `localhost` | Redis host |
| `c360_PROMOTION_REDIS_PORT` | `6580` | Redis port |
| `c360_PROMOTION_REDIS_DB` | `0` | Redis database number |
| `c360_PROMOTION_REDIS_PASSWORD` | unset | Redis password |
| `c360_PROMOTION_CACHE_TTL_SECONDS` | `60` | Cache TTL |
...
- Verify `c360_PROMOTION_DB_HOST` is reachable: `ping localhost`
...
### c360 Promotions already running
