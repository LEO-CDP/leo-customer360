...
#   c360_PROMOTION_API_HOST         c360 Promotions API listen address (default: localhost)
#   c360_PROMOTION_API_PORT         c360 Promotions API listen port (default: 9009)
#   c360_PROMOTION_UVICORN_RELOAD   Enable auto-reload (default: false)
###############################################################################
...
for arg in "$@"; do
    case "$arg" in
        --seed-demo-customer360-promotions)
            SEED_DEMO=true
            ;;
        --help|-h)
...
HOST="${c360_PROMOTION_API_HOST:-localhost}"
PORT="${c360_PROMOTION_API_PORT:-9009}"

c360_PROMOTION_DB_HOST="${c360_PROMOTION_DB_HOST:-localhost}"
c360_PROMOTION_DB_PORT="${c360_PROMOTION_DB_PORT:-5432}"
c360_PROMOTION_DB_USER="${c360_PROMOTION_DB_USER:-postgres}"
c360_PROMOTION_DB_PASSWORD="${c360_PROMOTION_DB_PASSWORD:-change_me_postgres_password}"
c360_PROMOTION_DB_NAME="${c360_PROMOTION_DB_NAME:-customer360}"
c360_PROMOTION_DB_CONTAINER_NAME="${c360_PROMOTION_DB_CONTAINER_NAME:-customer360-postgres}"
...
check_db_available() {
    "$VENV_PYTHON" - "$c360_PROMOTION_DB_HOST" "$c360_PROMOTION_DB_PORT" <<'PY'
import socket
import sys
...
run_sql_query() {
    ...
    if command -v psql >/dev/null 2>&1; then
        PGPASSWORD="$c360_PROMOTION_DB_PASSWORD" \
            psql \
            -h "$c360_PROMOTION_DB_HOST" \
            -p "$c360_PROMOTION_DB_PORT" \
            -U "$c360_PROMOTION_DB_USER" \
            -d "$c360_PROMOTION_DB_NAME" \
            -tAc "$sql" 2>/dev/null
        return
    fi

    if command -v docker >/dev/null 2>&1; then
        docker exec -i -u postgres "$c360_PROMOTION_DB_CONTAINER_NAME" \
            env PGPASSWORD="$c360_PROMOTION_DB_PASSWORD" \
            psql \
            -h 127.0.0.1 \
            -p 5432 \
            -U "$c360_PROMOTION_DB_USER" \
...
run_sql_file() {
    ...
    if ! docker cp "$sql_file" "$c360_PROMOTION_DB_CONTAINER_NAME:/tmp/$remote_name" >/dev/null 2>&1; then
        return 1
    fi

    if ! docker exec -i -u postgres "$c360_PROMOTION_DB_CONTAINER_NAME" \
        env PGPASSWORD="$c360_PROMOTION_DB_PASSWORD" \
        psql \
        -h 127.0.0.1 \
        -p 5432 \
        ...
        -f "/tmp/$remote_name" \
        >> "$LOG_FILE" 2>&1; then
        docker exec -u postgres "$c360_PROMOTION_DB_CONTAINER_NAME" rm -f "/tmp/$remote_name" >/dev/null 2>&1 || true
        return 1
    fi
...
if ! check_db_available; then
    log "${YELLOW}[DB] PostgreSQL not reachable${NC} | ${c360_PROMOTION_DB_HOST}:${c360_PROMOTION_DB_PORT}"

    if ensure_dev_infra; then

        if ! check_db_available; then
            log "${RED}[DB] PostgreSQL did not become ready${NC} | ${c360_PROMOTION_DB_HOST}:${c360_PROMOTION_DB_PORT}"
            exit 1
        fi
...
log "${GREEN}[DB] PostgreSQL ready${NC} | ${c360_PROMOTION_DB_HOST}:${c360_PROMOTION_DB_PORT}/${c360_PROMOTION_DB_NAME}"
...
if [ "$SEED_DEMO" = true ]; then
    ...
    log "[SEED] Use ./start.sh --seed-demo-customer360-promotions to initialize/seed demo data"
fi
...
log "${GREEN}[API] Starting c360 PROMOTION API API${NC}"
