#!/usr/bin/env bash
###############################################################################
# c360 PROMOTION API API -- Unit Test Runner
#
...
###############################################################################
# Load .env so c360_PROMOTION_DB_* variables are available for the PostgreSQL check below
###############################################################################
...
if command -v psql &> /dev/null; then
    if PGPASSWORD="${c360_PROMOTION_DB_PASSWORD:-}" psql -h "${c360_PROMOTION_DB_HOST:-localhost}" -p "${c360_PROMOTION_DB_PORT:-5432}" -U "${c360_PROMOTION_DB_USER:-postgres}" -d "${c360_PROMOTION_DB_NAME:-customer360}" -tAc "SELECT to_regclass('leo_ads.tenant') IS NOT NULL" -w 2>/dev/null | grep -qx "t"; then
...
# Run tests
###############################################################################
echo -e "${YELLOW}Running c360 PROMOTION API API unit tests...${NC}"
