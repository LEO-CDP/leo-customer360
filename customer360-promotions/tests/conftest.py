"""
Pytest configuration and shared fixtures for customer360-promotions tests.
...
def _pg_url() -> str:
    ...
    return (
        f"postgresql+psycopg2://{os.getenv('c360_PROMOTION_DB_USER', 'postgres')}:"
        f"{os.getenv('c360_PROMOTION_DB_PASSWORD', 'postgres')}@"
        f"{os.getenv('c360_PROMOTION_DB_HOST', 'localhost')}:"
        f"{os.getenv('c360_PROMOTION_DB_PORT', '5432')}/"
        f"{os.getenv('c360_PROMOTION_DB_NAME', 'customer360')}"
    )
