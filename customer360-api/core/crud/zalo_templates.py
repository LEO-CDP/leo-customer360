"""ZNS template sync — reuses ``crm_email_templates`` (metadata.channel='zalo_zns').
...
``crm_email_templates`` row (channel/id/params in ``metadata``/``variables``), so
...
from core.models.crm import EmailTemplate
...
def _map_status(remote: dict) -> str:
    """Map a Zalo template status onto the crm_email_templates lifecycle."""
...
def get_template(db: Session, tenant_id: uuid.UUID, template_id: uuid.UUID) -> Optional[EmailTemplate]:
