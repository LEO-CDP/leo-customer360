...
OA credentials/tokens are stored per-tenant in ``sys_data_source`` (slug
='zalo-oa'), not a dedicated table (see ``core.crud.zalo_oa``). These endpoints
...
def get_zalo_oauth_url(request: Request) -> ZaloOauthUrlResponse:
    """Build the Zalo OA consent URL (carries a signed, tenant-bound state)."""
    tenant_id = require_tenant(request)
    require_tenant_admin(request, "zalo oauth url")
    return ZaloOauthUrlResponse(authorize_url=zalo_oa.build_authorize_url(tenant_id))
...
def sync_zns_templates(request: Request, db: Session = Depends(get_db)) -> dict:
    ...
    tokens = (cfg.access_tokens or {}) if cfg is not None else {}
    ...
    return zalo_templates.sync_templates(
        db, uuid.UUID(tenant_id), tokens.get("oa_id"), tokens["access_token"]
    )
