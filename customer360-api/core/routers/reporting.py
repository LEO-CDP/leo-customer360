...
def get_cir_summary(
    ...
    by_status = identity_crud.raw_profiles_by_status(db, tenant_id, days=days)
    status_counts = {row["status_code"]: row["count"] for row in by_status}

    return CirSummary(
        total_raw_profiles=identity_crud.count_raw_profiles(db, tenant_id, days=days),
        total_master_profiles=identity_crud.count_master_profiles(db, tenant_id, days=days),
        ...
        duplicate_master_profile_count=identity_crud.count_duplicate_master_profiles(db, tenant_id, days=days),
        raw_profiles_by_status=by_status,
        raw_profiles_by_domain=identity_crud.raw_profiles_by_domain(db, tenant_id, days=days),
        master_profiles_by_domain=identity_crud.master_profiles_by_domain(db, tenant_id, days=days),
        raw_profiles_by_source_system=identity_crud.raw_profiles_by_source_system(db, tenant_id, days=days),
    )
...
def get_duplicate_master_profiles(
    ...
    return identity_crud.list_duplicate_master_profiles(db, tenant_id, skip=skip, limit=limit, days=days)
...
def get_identity_graph_coverage(
    ...
    return identity_crud.identity_graph_coverage(db, tenant_id, days=days)
