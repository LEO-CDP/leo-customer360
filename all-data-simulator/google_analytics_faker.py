...
# Common Google Analytics event types
GA_EVENTS = ['ga_install', 'ga_app_opened', 'ga_add_to_cart', 'ga_purchase', 'ga_complete_registration']
MEDIA_SOURCES = ['organic', 'googleadwords_int', 'Facebook Ads', 'Apple Search Ads']

def generate_google_analytics_event(days_ago=0):
    """
    Generates a single synthetic Google Analytics event payload.
    ...
    event_name = random.choice(GA_EVENTS)
    
    # Simulate a realistic purchase value if the event is a purchase
    event_value = round(random.uniform(10.0, 500.0), 2) if event_name == 'ga_purchase' else None
    event_currency = "USD" if event_name == 'ga_purchase' else None
...
if __name__ == "__main__":
    # Generate and print 5 sample Google Analytics events
    for _ in range(5):
        print(json.dumps(generate_google_analytics_event(), indent=2))